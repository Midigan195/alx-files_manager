...
async function thumbNail(width, localPath) {
  const thu = await imageThumbnail(localPath, { width });
  return thu;
}

fileQueue.process(aync (job, done) => {
  console.log('Loading...');
  ...
  const objectId = new ObjectID(field)

  files.findOne({ _id: objectId }, async (err, file) => {
    if (!file) {
